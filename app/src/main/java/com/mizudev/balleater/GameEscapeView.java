...
public class GameEscapeView extends SurfaceView implements Runnable{
    ...
    public GameEscapeView(GameEscapeBalls context, int x, int y){
        ...
        level = 1;
        score = 0;
        maxBallSize = Math.max(screenX, screenY);
        spawnFood();
    }
    ...
    private void update()
    {
        ...
        {
            // The player is hit by food - grow and check if the ball now
            // fills the screen. Afterwards spawn a new food item.
            player.grow(food.getFoodSize());
            if(player.getPlayerSize() * 2 >= maxBallSize){
                gameOver = true;
            } else {
                spawnFood();
            }
